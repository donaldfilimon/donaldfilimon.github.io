...
**The catalog registry is vendored.** `content/registry/projects.toml` is a publication-only snapshot of the former sibling `project-registry` (moved to `~/Archive/experimental-2026-09-18/` on 2026-09-18), keeping only the fields `scripts/sync-project-catalog.ts` reads: published projects carry `display_name`, `project_kind`, `lifecycle`, `ownership`, `ecosystems`, excluded projects only `stable_id`. This repo is public, so never paste the full registry here (it carries absolute paths and `export_policy = "private_only"`). To add a project, add its `[[projects]]` entry and its publication decision together, then `bun run sync:projects`; `--registry <path>` still overrides the default.
